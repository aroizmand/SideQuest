-- SideQuest Migration 010: Widen feed time window
-- 72 hours was too restrictive — users couldn't discover events next weekend.
-- Extending to 14 days so the feed stays useful for planning ahead.
...
WHERE q.status = 'active'
  AND q.starts_at > NOW()
  AND q.starts_at <= NOW() + INTERVAL '14 days';
